...
function loadProgressBar(status, cur, Total) {
  ...
  var a = Math.round(w);
  var y = Number(cur) * a;
  document.getElementById("myRequestProgress").innerHTML =
    '<div class="progress-bar progress-bar-striped progress-bar-animated bg-success" aria-valuenow="' +
    y +
...
function loadRequestDetails(qId) {
  ...
  var request = $.ajax({
    url:
      siteUrl +
      "/_api/web/lists/getbytitle('ccRequestTracker')/items(" +
      qId +
      ")",
    type: "GET",
  ...
  getCardHolder.then(function data() {
    $.when(request)
      .done(function(data) {
        ...
        // Reviews
        loadReviewTab(
          data.d.DIRECTORATE_APPROVAL,
          [
            "#directorateComments",
            "#directorateSignature",
            "#directorateReview"
          ],
          ["directorateComment", "directorateSignature", "directorateStatus"],
          "#directorateText"
        );
        loadReviewTab(
          data.d.BILLING_OFFICIAL_APPROVAL,
          ["#boComments", "#boSignature", "#boReview"],
          ["boComment", "boSignature", "boStatus"],
          "#billingOfficialText"
        );
        loadReviewTab(
          data.d.J6_APPROVAL,
          ["#j6Comments", "#j6Signature", "#j6Review"],
          ["j6Comment", "j6Signature", "j6Status"],
          "#j6Text"
        );
        loadReviewTab(
          data.d.PBO_APPROVAL,
          ["#pboComments", "#pboSignature", "#pboReview"],
          ["pboComment", "pboSignature", "pboStatus"],
          "#propertyBookText"
        );
        loadReviewTab(
          data.d.BUDGET_OFFICER_APPROVAL,
          [
            "#budgetOfficerComments",
            "#budgetOfficerSignature",
            "#budgetOfficerReview"
          ],
          [
            "budgetOfficerComment",
            "budgetOfficerSignature",
            "budgetOfficerStatus"
          ],
          "#budgetOfficerText"
        ...
        loadReviewTab(
          ...
          ["j8Comment", "j8Signature", "j8Status", "j8FiscalYear", "j8Quater"],
          "#j8Text"
        );
        loadReviewTab(
          data.d.CARD_HOLDER_VALIDATION,
          [
            "#cardHolderComments",
        ...
        loadReviewTab(
          data.d.REQUESTOR_VALIDATION,
          ["#requestorComments", "#requestorSignature"],
          ["requestorComment", "requestorSignature"],
          "#requestorText"
        );
        loadReviewTab(
          data.d.SUPPLY_VALIDATION,
          ["#supplyComments", "#supplySignature"],
          ["supplyComment", "supplySignature"],
          "#supplyText"
        );
        loadReviewTab(
          data.d.FINAL_VALIDATION,
          ["#j4Comments", "#j4Signature"],
          ["j4Comment", "j4Signature"],
          "#j4Text"
        );
        // Update bar
...
function loadDetailsDom(requestDetails) {
  ...
  for (var counter = 0; counter < requestDetails.Details.length; counter++) {
    ...
    $("#Description" + counter).val(
      requestDetails.Details[counter].requestDesc
    );
    $("#DD" + counter).prop(
      "checked",
      isBool(requestDetails.Details[counter].requestDdForm)
    );
    $("#DA" + counter).prop(
      "checked",
      isBool(requestDetails.Details[counter].requestDaForm)
    );
    $("#Source" + counter).val(requestDetails.Details[counter].requestSrc);
    $("#RequestCost" + counter).val(
      requestDetails.Details[counter].requestCost
    );
    $("#RequestTotal" + counter).val(
      requestDetails.Details[counter].requestTotal
    );
...
function closeRequest() {
  ...
  $().SPServices({
    ...
    completefunc: function(xData, Status) {
      console.log(
        'closeRequest: Changed request status to "CLOSED" to ID# ' + qId
      );
    }
  });
}
  ...
  $.ajax({
    ...
    success: function(data, textStatus, xhr) {
      $.each(data.d.results, function(i, item) {
        ...
        var b =
          "<a id='attachedFile' class='btn btn-link text-danger' onclick=\"deleteMyAttachment('" +
          qId +
          "','" +
          item.FileName +
...
function deleteMyAttachment(QID, fileName) {
  $.ajax({
    url:
      siteUrl +
      "/_api/web/lists/getbytitle('ccRequestTracker')/getItemById('" +
      QID +
...
function setValue(statusValue, isJ6) {
  for (var i = 0; i < stepStatus.length; i++) {
    if (stepStatus[i].caseStep === statusValue) {
      loadProgressBar(
        stepForwardStatus(statusValue, isJ6),
        stepStatus[i].numerStep,
        "10"
      );
    }
  }
...
function loadRowDetails(startCounter) {
  var counter;
  typeof startCounter === "undefined"
    ? (counter = 0)
    : (counter = startCounter.Details.length);
...
function rowDetailsColHtml(counter, cols) {
  ...
  cols +=
    '<td><input id="RequestQTY' +
    counter +
    '" type="text" class="form-control form-control-sm" name="qty' +
    counter +
    '"/></td>';
  cols +=
    '<td><input id="Description' +
    counter +
    '" type="text" class="form-control form-control-sm" name="Desc' +
    counter +
    '"/></td>';
  cols +=
    '<td><input id="Source' +
    counter +
    '" type="text" class="form-control form-control-sm" name="Src' +
    counter +
    '"/></td>';
  cols +=
    '<td><input id="RequestCost' +
    counter +
    '" type="text" onchange="updateForm(\'' +
    counter +
    '\')" class="form-control form-control-sm" name="Cost' +
    counter +
    '"/></td>';
  cols +=
    '<td><input id="Rate' +
    counter +
    '" type="text"     		class="form-control form-control-sm" name="Rate' +
    counter +
    '"/></td>';
  cols +=
    '<td><input id="DD' +
    counter +
    '"   type="checkbox"  		class="form-control form-control-sm" name="DD' +
    counter +
    '" style="border:none;border-radius:0px;"/></td>';
  cols +=
    '<td><input id="DA' +
    counter +
    '"   type="checkbox"  		class="form-control form-control-sm" name="DA' +
    counter +
    '" style="border:none;border-radius:0px;"/></td>';
  cols +=
    '<td><input id="RequestTotal' +
    counter +
    '" type="text" 	class="form-control form-control-sm" name="Total' +
    counter +
    '" readonly="readonly"/></td>';
  cols +=
    '<td><input type="button"  id="btnDel-' +
    counter +
    '" 		class="ibtnDel btn btn-sm btn-danger" style="width:100%"  value="&#10060; Delete" onclick="enableDeleteBtn(this)"></td>';
  return cols;
}
...
function signRequest(reviewStep) {
  ...
  for (var i = 0; i < returnedStep.length; i++) {
    if (returnedStep[i].stepName === reviewStep) {
      $(returnedStep[i].domId).val(
        "SIGNED BY: " + getUserName().Name + " ON: " + now
      );
    }
  }
}
...
function setApprovalProcess(reviewStep) {
  for (var i = 0; i < returnedStep.length; i++) {
    if (returnedStep[i].stepName === reviewStep) {
      submitReview(
        returnedStep[i].stepStatus,
        createJsonResponse(returnedStep[i].stepArray)
      );
    }
  }
}
...
function loadReviewTab(tabResponse, domArray, responseArray, tabText) {
  if (typeof tabResponse !== "undefined" && tabResponse !== null) {
    ...
    $(domArray[1]).val(
      decodeURIComponent(JSON.parse(tabResponse)[responseArray[1]])
    );
    $(domArray[2]).val(
      decodeURIComponent(JSON.parse(tabResponse)[responseArray[2]])
    );
    $(domArray[3]).val(
      decodeURIComponent(JSON.parse(tabResponse)[responseArray[3]])
    );
...
function cardMessage() {
  $("#RequestCardType").change(function() {
    ...
    switch (choice) {
      ...
      case "ORF":
        $("#warnoText").text(
          "The ORF card requires additional legal signatures. Ensure proper guidance. "
        );
        $("#warnoText").append(
          '<span class="badge badge-secondary"><a href="https://sof.hq.socom.mil/sites/SOCS/SJS/Pubs/_layouts/15/WopiFrame.aspx?sourcedoc=/sites/SOCS/SJS/Pubs/USSOCOM%20IMTs/22.pdf&action=default" target="_blank" style="color:white;">Download Form</a></span>'
        );
        break;
      case "Training":
        $("#warnoText").text(
          "This card requires an SF-182 for each attendee and legal fill ratios. "
        );
        $("#warnoText").append(
          '<span class="badge badge-secondary"><a href="../_layouts/15/WopiFrame.aspx?sourcedoc=/app/GPC/Shared%20Documents/SF182-06.pdf&action=default" target="_blank" style="color:white;">Download Form</a></span>'
        );
...
function addGrandTotal(numberData) {
  ...
  value > 9999
    ? $("#grandTotal").append(
        ' for request over 10K <span class="badge badge-secondary"><a href="../_layouts/15/WopiFrame.aspx?sourcedoc=/app/GPC/Shared%20Documents/RSCA%20Version%202.0%20Jan%202017.pdf&action=default" target="_blank" style="color:white;">Download Form</a></span>'
      )
    : false;
}
...
function getFiscalYear() {
  for (var i = 0; i < fiscalYear.length; i++) {
    $("#j8FiscalYear").append(
      '<option value="' + fiscalYear[i] + '">' + fiscalYear[i] + "</option>"
    );
  }
}
