...
var returnedStep = [
  ...
  {
    stepName: "budget",
    stepStatus: "BUDGET_OFFICER_APPROVAL",
    stepArray: [
      "budgetOfficerComment",
      "budgetOfficerStatus",
      "budgetOfficerSignature"
    ],
    domId: "#budgetOfficerSignature"
  },
  {
    stepName: "j8",
...
var getCommandData = $.ajax({
  url:
    siteUrl +
    "/_api/web/lists/getbytitle('ccCommand')/Items?$Select=Title, COMMAND_URL, COMMAND_WARNING, COMMAND_CLASSIFICATION",
  type: "GET",
...
var personInformation = function() {
  var information = {
    ...
    pseudoName:
      $("#personRole option:selected").text() +
      $("#personLName").val() +
      $("#personFName").val()
  };
  return information;
...
function getUsersListHtml(item) {
  $("#usersList").append(
    "<tr>\
      ...
      item.PERSON_EMAIL +
      '</td>\
				<td><a href="#" onclick="setUserInformationRedirect(' +
      item.Id +
      ');">' +
      item.PERSON_DIRECTORATE +
      "</td>\
			</tr>"
  );
...
var getApprovalData = function() {
  var ApprovalData = {
    ...
    budgetOfficerSignature: encodeURIComponent(
      $("#budgetOfficerSignature").val()
    ),
    /* J8 section */
...
var getCardHolderBillingApprover = function() {
  ...
  $.each(getUser.responseJSON.d.results, function(index, value) {
    if (
      getUser.responseJSON.d.results[index].PERSON_EMAIL ===
        requestNotification.RequestorCardHolderName &&
      getUser.responseJSON.d.results[index].PERSON_ROLE === "CARD HOLDER"
    ) {
      console.log(getUser.responseJSON.d.results[index]);
      id = index;
      billingOfficialArray["billingOfficial"] = JSON.parse(
        getUser.responseJSON.d.results[id].PERSON_ATTRIBUTES
      ).billingOfficial;
      billingOfficialArray["directorAprove"] =
        getUser.responseJSON.d.results[id].PERSON_DIRECTORATE;
...
var getDirectorateApprover = function() {
  ...
  $.each(getUser.responseJSON.d.results, function(index, value) {
    if (
      getUser.responseJSON.d.results[index].PERSON_DIRECTORATE ===
        getCardHolderBillingApprover().directorAprove &&
      getUser.responseJSON.d.results[index].PERSON_ROLE ===
        "DIRECTORATE APPROVAL"
    ) {
...
function getRequestsListHtml(item) {
  $("#requestList").append(
    "<tr>\
      ...
      "</td>\
					<td>" +
      JSON.parse(item.REQUEST_FIELD).RequestorDirectorate +
      "</td>\
					<td>" +
      getFiscalInformation(item.J8_APPROVAL, "year") +
      "</td>\
					<td>" +
      getFiscalInformation(item.J8_APPROVAL, "quater") +
      "</td>\
					<td>" +
      JSON.parse(item.REQUEST_FIELD).RequestJustification +
      '</td>\
					<td><a href="#" onclick="setRequestInformationRedirect(' +
      item.Id +
      ')">' +
      stepForwardStatus(
        item.REQUEST_STATUS,
        JSON.parse(item.REQUEST_FIELD).RequestIsJ6
      ) +
      "</td>\
				</tr>"
  );
...
function getTrainingListHtml(item) {
  $("#usersTraining").append(
    '<tr id="' +
      ...
      '"  name"' +
      item.COURSE_CODE +
      '" class="inputTextCCUser form-control training">\
						<div>\
					</td>\
			</tr>'
  );
...
function getUserInformationHtml(item) {
  ...
  $("#billingOfficialCardHolder")
    .select2()
    .val(attributes.billingOfficial)
    .trigger("change");
...
function getUserHtml(item) {
  ...
  item.PERSON_ROLE === "CARD HOLDER"
    ? $("#RequestCardHolderName").append(
        '<option value="' +
          item.PERSON_EMAIL +
          '">' +
          item.PERSON_EMAIL +
  ...
  item.PERSON_ROLE === "BILLING OFFICIAL"
    ? $("#billingOfficialCardHolder").append(
        '<option value="' +
          item.PERSON_EMAIL +
          '">' +
          item.PERSON_EMAIL +
...
function getUserPanesHtml() {
  ...
  e1 !== "CARD HOLDER"
    ? $("#attributes_main").fadeOut()
    : $("#attributes_main").fadeIn();
  e1 !== "CARD HOLDER"
    ? $("#billing_official_main").fadeOut()
    : $("#billing_official_main").fadeIn();
  e1 === "CARD HOLDER"
    ? $("#training_main").fadeIn()
    : e1 === "BILLING OFFICIAL"
    ? $("#training_main").fadeIn()
    : $("#training_main").fadeOut();
}
...
function getFiscalInformation(jsonData, type) {
  if (typeof jsonData !== "undefined" && jsonData !== null) {
    ...
    type === "year"
      ? (parsedReturn = parsedJson.j8FiscalYear)
      : (parsedReturn = parsedJson.j8Quater);
  } else {
...
function stepForwardStatus(status, isJ6) {
  ...
  for (var i = 0; stepStatus.length > i; i++) {
    ...
    isJ6 === "Yes"
      ? status === stepStatus[i].caseStep
        ? (forwardStatus = stepStatus[i].fwdj6)
        : false
      : status === stepStatus[i].caseStep
      ? (forwardStatus = stepStatus[i].fwd)
      : false;
  }
...
function getRole() {
  for (var i = 0; i < personType.length; i++) {
    $("#personRole").append(
      '<option value="' + personType[i] + '">' + personType[i] + "</option>"
    );
  }
}
...
var setCurrency = function() {
  var returnCurrent;
  $("#RequestCurrencyType").is(":checked") === true
    ? (returnCurrent = "Dollar")
    : (returnCurrent = "Euro");
...
function setUserInformationRedirect(userId) {
  window.open(
    "../Pages/cc_user_edit" + fileExt + "?PersonId=" + userId,
    "_self"
  );
}
...
function setRequestInformationRedirect(requestId, redirectArgument) {
  window.open(
    "../Pages/purchase_request" + fileExt + "?id=" + requestId,
    "_blank"
  );
}
/*
...
function setPrintViewRedirect(requestId, redirectArgument) {
  window.open(
    "../Pages/purchase_request_print" + fileExt + "?id=" + requestId,
    "_blank"
  );
}
/*
